...
public enum UserAgent {
    ...
    private static final Map<UserAgentType, List<UserAgent>> MAP_AGENTS = new EnumMap<>(UserAgentType.class);
    
    static {
        for (UserAgent userAgent: UserAgent.values()) {
            
            if (MAP_AGENTS.get(userAgent.type) == null) {
                MAP_AGENTS.put(userAgent.type, new ArrayList<>());
            }
            
            MAP_AGENTS.get(userAgent.type).add(userAgent);
        }
    }
