...
public class StrategyTime extends AbstractStrategy {
    ...
    public void checkApplicability() throws StoppedByUserSlidingException {
        if (this.injectionModel.getMediatorUtils().preferencesUtil().isStrategyTimeDisabled()) {
            ...
            this.injectionModel.getMediatorEngine().getEngine().instance().getModelYaml().getStrategy().getBinary().getTime()
        )) {
            LOGGER.log(
    ...
    public void allow(int... i) {
        this.injectionModel.appendAnalysisReport(
            ...
            + this.injectionModel.getReportWithoutIndex(
                this.injectionModel.getMediatorEngine().getEngine().instance().sqlTestTimeWithOperator(
                    this.injectionModel.getMediatorEngine().getEngine().instance().sqlTime(StringUtil.formatReport(LogLevelUtil.COLOR_GREEN, "&lt;query&gt;"), "0", true),
    ...
    public String inject(String sqlQuery, String startPosition, AbstractSuspendable stoppable, String metadataInjectionProcess) throws StoppedByUserSlidingException {
        return this.injection.inject(
            this.injectionModel.getMediatorEngine().getEngine().instance().sqlTime(sqlQuery, startPosition, false),
            stoppable
        );
    }
