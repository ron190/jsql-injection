...
public class MarkErrorVulnerable implements InteractionCommand {
    ...
    @Override
    public void execute() {
        LOGGER.info(InteractionCommand.addGreenColor(this.injectionModel.mediatorVendor.getVendor().instance().getXmlModel().getStrategy().getError().getMethod().get(this.indexMethodError).getName()));
    }
