...
public class MarkErrorInvulnerable implements InteractionCommand {
    ...
    @Override
    public void execute() {
        LOGGER.info(InteractionCommand.addRedColor(this.injectionModel.getMediatorVendor().getVendor().instance().getXmlModel().getStrategy().getError().getMethod().get(this.indexMethodError).getName()));
    }
