...
public class ConcreteBlindInjection extends AbstractBlindInjection<CallableBlind> {
    ...
    public ConcreteBlindInjection() {
        ...
        if (MediatorModel.model().isProcessStopped) {
            return;
        }
    ...
    public boolean isInjectable() throws PreparationException {
        ...
        if (MediatorModel.model().currentVendor.getValue().getSqlBlindFirstTest() == null) {
            return false;
        }
        
        CallableBlind blindTest = new CallableBlind(MediatorModel.model().currentVendor.getValue().getSqlBlindFirstTest());
        try {
            blindTest.call();
        } catch (Exception e) {
            LOGGER.error(e, e);
