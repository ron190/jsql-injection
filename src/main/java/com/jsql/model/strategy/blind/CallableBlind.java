...
import com.jsql.model.strategy.blind.diff_match_patch.Diff;
...
public class CallableBlind extends CallableAbstractBlind<CallableBlind> {
    ...
    public CallableBlind(String inj) {
        this.blindUrl = MediatorModel.model().currentVendor.getValue().getSqlBlindCheck(inj);
    }
    ...
    public CallableBlind(String inj, int indexCharacter, int bit) {
        this.blindUrl = MediatorModel.model().currentVendor.getValue().getSqlBlindBitCheck(inj, indexCharacter, bit);
    ...
    public CallableBlind(String inj, int indexCharacter, boolean isTestingLength) {
        this.blindUrl = MediatorModel.model().currentVendor.getValue().getSqlBlindLengthCheck(inj, indexCharacter);
